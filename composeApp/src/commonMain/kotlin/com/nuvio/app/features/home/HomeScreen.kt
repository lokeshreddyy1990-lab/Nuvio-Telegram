...
fun HomeScreen(
    ...
    BoxWithConstraints(
        ...
        val infoRichHeroEnabled = nuvioEnhancedSettings.heroDisplayMode == NuvioHeroDisplayMode.InfoRich
        val heroToContinueWatchingGap = HOME_STREAMING_SHOWCASE_HERO_TO_CONTINUE_WATCHING_GAP
                    ...
                    item {
                        when {
                            showHeroSkeleton -> HomeSkeletonHero(
                                ...
                                posterArtHeroEnabled = false,
                                streamingShowcaseHeroEnabled = true,
                            )
                            ...
                            homeUiState.heroItems.isNotEmpty() -> HomeHeroSection(
                                ...
                                heroDisplayMode = if (infoRichHeroEnabled) {
                                    NuvioHeroDisplayMode.InfoRich
                                } else {
                                    NuvioHeroDisplayMode.Balanced
                                },
                                heroArtworkSource = NuvioHeroArtworkSource.Backdrop,
                                posterArtHeroEnabled = false,
                                streamingShowcaseHeroEnabled = true,
                                ...
                                compactMetadata = !infoRichHeroEnabled || nuvioEnhancedSettings.compactHeroMetadata,
                                showRatings = infoRichHeroEnabled && nuvioEnhancedSettings.showHeroRatings,
                                showOverview = infoRichHeroEnabled && nuvioEnhancedSettings.showHeroOverview,
                                ...
                                if (section != null && section.items.isNotEmpty()) {
                                    item(key = settingsItem.key) {
                                        HomeCatalogRowSection(
                                            section = section,
                                            entries = section.items.take(
                                                HOME_CATALOG_PREVIEW_LIMIT * homeSettingsUiState.homeCatalogRowCount,
                                            ),
                                            modifier = Modifier.padding(bottom = 12.dp),
                                            sectionPadding = homeSectionPadding,
                                            hideReleaseDates = nuvioEnhancedSettings.hideHomeReleaseDates,
                                            onViewAllClick = if (
                                                section.canOpenCatalog(
                                                    HOME_CATALOG_PREVIEW_LIMIT * homeSettingsUiState.homeCatalogRowCount,
                                                )
                                            ) {
